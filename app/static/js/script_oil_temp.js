// Oil Temperature Chart Module
const OilTemperatureChart = (function() {
  // Chart configuration
  const chartConfig = {
  ...
  function initialize(elementId) {
    ...
    // Legend - moved below title, horizontally arranged
    const legend = svg
      .append("g")
      .attr("class", "legend")
  ...
  function update(chart, data, selectedMonth, sigmaLevel, startDate, endDate) {
    ...
    if (selectedMonth !== "all") {
      ...
      filteredData = filteredData.filter((row) => row.month === monthNum);
    }

    // Check if we have data after filtering
    if (filteredData.length === 0) {
      console.warn(`No data available for Oil Temperature with the selected filters`);
      return;
    }
    ...
    const dataMax = d3.max(filteredData, (d) => d.Oil_temperature);
    ...
    const dataMin = d3.min(filteredData, (d) => d.Oil_temperature);
    ...
    pointSelection
      ...
      .attr("cy", (d) => yScale(d.Oil_temperature));
    ...
    const itemWidth = 80; // Reduced width allocated for each legend item
  ...
  function addInteractiveFeatures(chart, data, xScale, yScale) {
    ...
    overlay
      ...
      .on("mousemove", function() {
        ...
        const yPos = yScale(d.Oil_temperature);
        ...
        chart.tooltip
          .html(
            `<strong>Oil Temperature</strong><br/>
                       Date: ${formatDate(d.date)}<br/>
                       Value: ${formatValue(d.Oil_temperature)}<br/>
                       Mean: ${formatValue(d.monthly_mean)}<br/>
                       Std Dev: ${formatValue(d.monthly_std_dev)}`
          )
